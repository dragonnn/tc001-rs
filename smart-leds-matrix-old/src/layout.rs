...
use invert_axis::{InvertX, InvertXY, InvertY, NoInvert, Tc001};
...
impl Rectangular<Tc001> {
    /// Creates a new rectangular layout with inverted X and Y axis.
    pub const fn new_tc001(width: u32, height: u32) -> Self {
        Self::new_common(width, height)
    }
}
...
impl Layout for Rectangular<Tc001> {
    fn map(&self, p: Point) -> Option<usize> {
        if p.x < 0 || p.y < 0 || p.x >= self.size.width as i32 || p.y >= self.size.height as i32 {
            return None;
        }
        if p.y % 2 == 0 {
            return Some(((p.y * 32) + p.x) as usize);
        }
        return Some(((p.y * 32) + (31 - p.x)) as usize);
    }
