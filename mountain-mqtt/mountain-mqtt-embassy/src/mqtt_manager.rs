...
use mountain_mqtt::{
    client::{
        Client, ClientError, ClientNoQueue, ClientReceivedEvent, ConnectionSettings, EventHandler, EventHandlerError,
    },
...
pub enum MqttEvent<E> {
    ...
    ApplicationEvent { connection_id: ConnectionId, event: E },
    ...
    Disconnected { connection_id: ConnectionId, error: Error },
...
impl<E> defmt::Format for MqttEvent<E> {
    fn format(&self, f: defmt::Formatter) {
        match self {
            Self::ApplicationEvent { connection_id, event: _ } => {
            ...
            Self::Disconnected { connection_id, error } => {
                defmt::write!(f, "Disconnected({}, {})", connection_id, error)
            }
            Self::SubscriptionGrantedBelowMaximumQos { connection_id, granted_qos, maximum_qos } => defmt::write!(
                f,
            ...
            Self::PublishedMessageHadNoMatchingSubscribers { connection_id } => {
                defmt::write!(f, "PublishedMessageHadNoMatchingSubscribers({})", connection_id)
            }
...
impl<A> State<A> {
    fn new() -> Self {
        Self { last_connection_event: Instant::now(), pending_action: None }
    }
...
struct ChannelEventHandler<'a, A, E, M, const P: usize, const Q: usize>
where
    E: FromApplicationMessage<P> + Clone,
    M: embassy_sync::blocking_mutex::raw::RawMutex,
{
    connection_id: ConnectionId,
    event_sender: &'a Sender<'a, M, MqttEvent<E>, Q>,
    state: &'a RefCell<State<A>>,
}

impl<A, E, M, const P: usize, const Q: usize> EventHandler<P> for ChannelEventHandler<'_, A, E, M, P, Q>
where
    E: FromApplicationMessage<P> + Clone,
    M: embassy_sync::blocking_mutex::raw::RawMutex,
{
    async fn handle_event(&mut self, event: ClientReceivedEvent<'_, P>) -> Result<(), EventHandlerError> {
        match event {
            ClientReceivedEvent::ApplicationMessage(message) => {
                let event = E::from_application_message(&message)?;
                self.event_sender.send(MqttEvent::ApplicationEvent { connection_id: self.connection_id, event }).await;
            }
            ClientReceivedEvent::Ack => {
                self.state.borrow_mut().record_connection_event();
            }
            ClientReceivedEvent::SubscriptionGrantedBelowMaximumQos { granted_qos, maximum_qos } => {
                self.event_sender
            ...
            ClientReceivedEvent::PublishedMessageHadNoMatchingSubscribers => {
                self.event_sender
                    .send(MqttEvent::PublishedMessageHadNoMatchingSubscribers { connection_id: self.connection_id })
                    .await
            }
            ClientReceivedEvent::NoSubscriptionExisted => {
                self.event_sender.send(MqttEvent::NoSubscriptionExisted { connection_id: self.connection_id }).await
            }
        }
        Ok(())
    }
...
{
    if let Err(e) = action.perform(client, connection_settings.client_id(), current_connection_id, is_retry).await {
...
async fn handle_messages<'a, A, C, E, M, const Q: usize>(
    current_connection_id: ConnectionId,
    client: &mut C,
    state: &RefCell<State<A>>,
    connection_settings: &ConnectionSettings<'static>,
    event_sender: &Sender<'static, M, MqttEvent<E>, Q>,
    action_receiver: &mut Receiver<'static, M, A, Q>,
    settings: &Settings,
) -> Result<(), Error>
where
    C: Client<'a>,
    ...
    M: embassy_sync::blocking_mutex::raw::RawMutex,
{
    ...
    event_sender.send(MqttEvent::Connected { connection_id: current_connection_id }).await;
    ...
    loop {
        ...
        if let Some(instant) = connection_instant {
            if instant.elapsed() > settings.stabilisation_interval {
                ...
                event_sender.send(MqttEvent::ConnectionStable { connection_id: current_connection_id }).await;
            }
        }
        ...
        if let Some(action) = pending_action {
            try_action(current_connection_id, client, state, connection_settings, action, true).await?;
        }

        // Handle actions from receiver
        while let Ok(action) = action_receiver.try_receive() {
            try_action(current_connection_id, client, state, connection_settings, action, false).await?;
        }
    }
}
...
/// module.
///
pub async fn run<A, E, M, const P: usize, const B: usize, const Q: usize>(
    stack: Stack<'static>,
    connection_settings: ConnectionSettings<'static>,
    settings: Settings,
    event_sender: Sender<'static, M, MqttEvent<E>, Q>,
    mut action_receiver: Receiver<'static, M, A, Q>,
) -> !
where
    E: FromApplicationMessage<P> + Clone,
    A: MqttOperations + Clone,
    M: embassy_sync::blocking_mutex::raw::RawMutex,
{
    ...
    loop {
        ...
        let event_handler = ChannelEventHandler { connection_id, event_sender: &event_sender, state: &state };

        let mut client = ClientNoQueue::new(connection, &mut mqtt_buffer, delay, timeout_millis, event_handler);
        ...
        .await
        {
            ...
            event_sender.send(MqttEvent::Disconnected { connection_id, error }).await;
        }
