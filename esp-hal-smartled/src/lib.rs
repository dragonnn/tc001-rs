    ...
    Timing: crate::Timing,
{
    ...
    fn create_rmt_data(&mut self, iterator: impl IntoIterator<Item = impl Into<C>>) -> Result<(), AdapterError> {
    ...
    #[allow(unused)]
    pub(crate) fn write_pixel_data(&mut self, index: usize, color: impl Into<C>) -> Result<(), AdapterError> {
        let buffer_start_index = index * C::CHANNELS as usize * (size_of::<C::ChannelType>() * 8);
        let mut buffer_iter =
            self.rmt_buffer.get_mut(buffer_start_index..).ok_or(AdapterError::BufferSizeExceeded)?.iter_mut();
    ...
    pub fn flush(&mut self) -> Result<(), AdapterError> {
        ...
        match transmit {
            Ok(t) => match t.wait() {
                ...
                Err(e) => {
                    self.channel = Some(e.1);
                    Err(AdapterError::TransmissionError(e.0))
                }
            },
            Err((e, chan)) => {
                self.channel = Some(chan);
                return Err(AdapterError::TransmissionError(e));
            }
        }
    }
}
