...
fn convert_rgb_channel_to_pulses(
    ...
    for position in [128, 64, 32, 16, 8, 4, 2, 1] {
        *mut_iter.next().ok_or(LedAdapterError::BufferSizeExceeded)? = match channel_value & position {
            0 => pulses.0,
            _ => pulses.1,
        }
    }
    ...
    Color: rgb::ComponentSlice<u8>,
{
        ...
        C: TxChannelCreator<'ch, Blocking>,
    {
        ...
        Self { channel: Some(channel), rmt_buffer, pulses: led_pulses_for_clock(src_clock), color: PhantomData }
    }
}

impl<'ch, const BUFFER_SIZE: usize, Color> SmartLedsWrite for SmartLedsAdapter<'ch, BUFFER_SIZE, Color>
where
    Color: rgb::ComponentSlice<u8>,
{
        ...
        I: Into<Self::Color>,
    {
        ...
        *seq_iter.next().ok_or(LedAdapterError::BufferSizeExceeded)? = PulseCode::end_marker();
        ...
        match channel.transmit(self.rmt_buffer).map_err(|e| e.0)?.wait() {
            Ok(chan) => {
    ...
    Color: rgb::ComponentSlice<u8>,
{
        ...
        C: TxChannelCreator<'ch, Async>,
    {
        ...
        Self { channel, rmt_buffer, pulses: led_pulses_for_clock(src_clock), color: PhantomData }
    }
    ...
    Color: rgb::ComponentSlice<u8>,
{
        ...
        I: Into<Self::Color>,
    {
        ...
        for chunk in self.rmt_buffer.chunks(RMT_RAM_ONE_LED + 1) {
            self.channel.transmit(chunk).await.map_err(LedAdapterError::TransmissionError)?;
        }
        Ok(())
    }
}
